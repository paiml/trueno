...
// ── Manually added for trueno local contracts (elementwise-kernel-v1, gemv-kernel-v1) ──

/// Preconditions for equation `add` (elementwise-kernel-v1)
/// Domain-specific. Call: `contract_pre_add!(a_expr, b_expr)`
macro_rules! contract_pre_add {
    () => {{}};
    ($a:expr, $b:expr) => {{
        debug_assert!(!$a.is_empty(), "Contract add: precondition violated — a is empty");
        debug_assert!(
            $a.len() == $b.len(),
            "Contract add: precondition violated — a.len() != b.len()"
        );
    }};
}

/// Preconditions for equation `gemv` (gemv-kernel-v1)
/// Domain-specific. Call: `contract_pre_gemv!(a_expr, x_expr)`
macro_rules! contract_pre_gemv {
    () => {{}};
    ($a:expr, $x:expr) => {{
        debug_assert!(!$a.is_empty(), "Contract gemv: precondition violated — a is empty");
        debug_assert!(!$x.is_empty(), "Contract gemv: precondition violated — x is empty");
    }};
}

// Total: 610 preconditions, 15 postconditions from 165 contracts
