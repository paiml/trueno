...
/// NF4 blockwise dequantization — GPU compute shader
/// Contract: nf4-dequantization-v1 / blockwise_dequant
/// Lean theorem: ProvableContracts.NF4.gpu_cpu_parity
///
/// Input: packed u8 bytes (2 nibbles per byte), absmax f32 per block
/// Output: f32 values (2 per input byte)
/// Dispatch: ceil(num_bytes / 256) workgroups
pub const NF4_DEQUANT_SHADER: &str = r#"
@group(0) @binding(0) var<storage, read> packed: array<u32>;  // packed bytes as u32
...
@group(0) @binding(3) var<uniform> params: NF4Params;

// NF4 codebook LUT (16 entries, quantiles of standard normal)
const NF4_LUT = array<f32, 16>(
    -1.0,
    -0.6961928,
    -0.5250731,
    -0.3949175,
...
@compute @workgroup_size(256)
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    let byte_idx = gid.x + gid.y * 65535u * 256u;
    if (byte_idx >= params.num_bytes) { return; }

    // Extract byte from u32 array (4 bytes per u32)
    let word_idx = byte_idx / 4u;
    let byte_offset = byte_idx % 4u;
    let word = packed[word_idx];
    let byte_val = (word >> (byte_offset * 8u)) & 0xFFu;

    // High nibble → output[2*byte_idx]
    let hi_nibble = (byte_val >> 4u) & 0x0Fu;
    let lo_nibble = byte_val & 0x0Fu;

    let elem_hi = byte_idx * 2u;
    let elem_lo = byte_idx * 2u + 1u;

    let block_hi = elem_hi / params.blocksize;
    let block_lo = elem_lo / params.blocksize;

    output[elem_hi] = NF4_LUT[hi_nibble] * absmax[block_hi];
    output[elem_lo] = NF4_LUT[lo_nibble] * absmax[block_lo];
}
"#;
