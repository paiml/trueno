...
fn main() {
    ...
    std::fs::write("/tmp/rmsnorm.ptx", &ptx).expect("failed to write rmsnorm PTX");
    println!("RMSNorm PTX written to /tmp/rmsnorm.ptx ({} bytes)", ptx.len());

    // Test Q5_0 kernel (this is the one failing)
    println!("=== Q5_0 GEMV Kernel ===");
    let q5 = Q5_0GemvKernel::new(896, 896);
    let ptx = q5.emit_ptx();
    std::fs::write("/tmp/q5_gemv.ptx", &ptx).expect("failed to write q5_gemv PTX");
    ...
    let ptx = sm.emit_ptx();
    std::fs::write("/tmp/softmax.ptx", &ptx).expect("failed to write softmax PTX");
    println!("Softmax PTX written to /tmp/softmax.ptx ({} bytes)", ptx.len());
}
