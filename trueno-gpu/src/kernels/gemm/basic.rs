...
mod tests {
    ...
    fn test_tiled_unrolled_emits_valid_ptx() {
        ...
        // Verify barriers (shared memory synchronization)
        assert!(ptx.contains("bar.sync"));
    ...
    fn test_tiled_unrolled_uses_shared_memory() {
        ...
        // Tiled GEMM uses shared memory for A and B tiles
        // Size = tile_size * tile_size * 4 * 2 = 16 * 16 * 4 * 2 = 2048 bytes
        assert!(ptx_kernel.shared_memory_bytes() > 0);
    ...
    fn test_tiled_unrolled_various_tile_sizes() {
        // Test with different tile sizes (must be divisible by 4 for unrolling)
    ...
    fn test_tiled_unrolled_shared_memory_calculation() {
        // Shared memory = tile_size^2 * 4 bytes * 2 (A and B tiles)
    ...
    fn test_tiled_unrolled_ptx_contains_fma() {
        ...
        // Should contain FMA (fused multiply-add) for computation
    ...
    fn test_tiled_unrolled_n_tiles_calculation() {
        ...
        // k=64, tile=16 -> n_tiles = 4
        // The kernel should work correctly
        assert!(ptx.contains(".entry gemm_tiled_unrolled"));
    }
    ...
    fn test_tiled_unrolled_k_not_divisible_by_tile() {
        // k=100, tile=16 -> n_tiles = 7 (with remainder handling)
    ...
    fn test_ptx_emission_structural_consistency() {
        // Note: PTX register declaration order may vary due to HashMap iteration
        // We verify structural consistency instead of exact string equality
        let kernel = GemmKernel::tiled_unrolled(64, 64, 64, 16);
        let ptx1 = kernel.emit_ptx();
        let ptx2 = kernel.emit_ptx();

        // Both emissions should have the same entry point
        ...
        // Both should have the same barriers
        assert_eq!(
            ptx1.matches("bar.sync").count(),
            ptx2.matches("bar.sync").count()
        );

        // Both should have same number of FMA instructions
        assert_eq!(
            ptx1.matches("fma.rn.f32").count(),
            ptx2.matches("fma.rn.f32").count()
        );

        // Same length (structural equivalence)
