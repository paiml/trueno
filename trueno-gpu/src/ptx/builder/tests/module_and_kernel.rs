...
fn test_module_builder() {
    let module = PtxModule::new().version(8, 5).target("sm_86").address_size(64);
...
fn test_kernel_params() {
    let kernel = PtxKernel::new("test").param(PtxType::U64, "ptr").param(PtxType::U32, "n");
...
#[test]
fn test_emit_kernel() {
    let kernel = PtxKernel::new("vector_add").param(PtxType::U64, "a").param(PtxType::U64, "b");
...
fn test_bar_sync_emission() {
    ...
    assert!(ptx.contains("bar.sync 0"), "Expected 'bar.sync 0' but got: {}", ptx);
    // Must NOT contain the buggy output
    assert!(!ptx.contains("bar.b32"), "Found buggy 'bar.b32' in: {}", ptx);
}
...
fn test_cvt_u64_u32_emission() {
    ...
    assert!(ptx.contains("cvt.u64.u32"), "Expected 'cvt.u64.u32' but got: {}", ptx);
}

#[test]
fn test_shared_memory_addressing() {
    // Shared memory access uses register-based addressing
    let kernel = PtxKernel::new("test_shared").shared_memory(1024).build(|ctx| {
    ...
    assert!(ptx.contains("[%rd"), "Expected bracketed register address, got: {}", ptx);
}
...
fn test_bar_sync_with_different_barriers() {
    ...
    let ptx = kernel.emit();
    assert!(ptx.contains("bar.sync 0"), "Expected 'bar.sync 0' in: {}", ptx);
    assert!(ptx.contains("bar.sync 1"), "Expected 'bar.sync 1' in: {}", ptx);
}

#[test]
fn test_global_memory_addressing() {
    // BUG: global memory access was "ld.global.f32 %f, %r" instead of "ld.global.f32 %f, [%r]"
    let kernel = PtxKernel::new("test_global").param(PtxType::U64, "ptr").build(|ctx| {
...
fn test_loop_counter_update_in_place() {
    ...
    let kernel = PtxKernel::new("test_loop").param(PtxType::U32, "n").build(|ctx| {
